...
class EigenTransition(Scene):
    ...
    def show_eigen_comparison(self):
        ...
        left_subheading = VGroup(
            ...
            Text("Linear Operator", font_size=18), 
        ...
        matrix_equation = MathTex(r"T(x,y) = (2x, 0.5y)", font_size=22)
        matrix_equation.move_to(LEFT*3.5 + UP*1.5)  # Matrix appears first

        discrete_eigen_eq = MathTex(r"\boldmath Tu = \lambda u", font_size=24, color=GOLD)
        ...
        connection_text = MathTex(r"\text{Same idea, different dimensions!}", 
                                 font_size=26, color=YELLOW)
        connection_text.move_to(ORIGIN + DOWN*3)

        left_mapping = MathTex(r"T: \mathbb{R}^2 \rightarrow \mathbb{R}^2", font_size=22)
    ...
    def show_scaling_comparison(self):
        ...
        # ---------- LEFT SIDE ----------
        left_title = Text("Linear Operator Action", font_size=28).move_to(LEFT_COL + TOP_Y)
        ...
        op_eq   = MathTex(r"L[u] = -\dfrac{d^2u}{dx^2}", font_size=26).next_to(dom_axes, DOWN, buff=0.5)
        eig_eq  = MathTex(r"L[u] = \lambda u",           font_size=26).next_to(op_eq  , DOWN, buff=0.3)
        lam_eq  = MathTex(r"\lambda = 1",                font_size=22).next_to(eig_eq, DOWN, buff=0.3)
    ...
    def show_orthogonality_comparison(self):
        ...
        # ------------- DEFINITIONS -------------
        dot_vec_def = MathTex(r"\text{Inner product: }u\cdot v=\sum_i u_i v_i",
                              font_size=24, color=GOLD).move_to(L_COL + TOP_Y - UP)
        dot_fun_def = MathTex(r" \text{Inner product: }\langle f,g\rangle=\int_0^L f(x)g(x)\,dx",
