...
def interactive_ode_demo():
    ...
    y0_slider = widgets.FloatSlider(
        value=2.0, min=-2, max=8, step=0.1,
        description="y(1):",
    ...
    def plot_ode(y0, show_solutions, show_field):
        with output:
            ...
            ax.plot(t, y_vals, "-", color="C3", linewidth=3,
                    label=fr"Solution $y(1)\,=\,{y0}$")

            # Mark the initial condition at t=1
            t0 = 1.0
            ax.plot([t0], [y0], "ro", markersize=8, label="Initial Condition")

            ax.legend(loc="upper left")
            plt.tight_layout()
            plt.show()

            # Analysis/info section (as in original)
            print("\nAnalysis:")
            print(f"• Initial condition:  y(1) = {y0}")
            print("• General solution:   y(t) = t² + c / t²")
            print(f"• Particular:         y(t) = t² + ({y0}-1)/t²")
